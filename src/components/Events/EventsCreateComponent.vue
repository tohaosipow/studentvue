    ...
    <v-container fluid>
        <v-row>
            <v-col cols="12" lg="7">
                <v-card>
                    <v-card-text>
                        <v-text-field label="Название" v-model="event.name"/>
                        <v-textarea label="Описание" v-model="event.description"/>
                        <v-switch label="Мероприятие проводится внутри университета" v-model="inner"/>

                        <v-text-field label="Адрес" v-if="!inner" v-model="address"/>
                        <v-autocomplete v-if="inner" :clearable="true"
                        ...
                        <v-text-field :placeholder="$moment().format('DD.MM.YYYY HH:mm')"
                        ...
                        <v-text-field :placeholder="$moment().format('DD.MM.YYYY HH:mm')" label="Начало мероприятия"
                                      v-mask="'##.##.#### ##:##'" v-model="event.start_at"/>
                        <v-text-field :placeholder="$moment().format('DD.MM.YYYY HH:mm')" label="Конец мероприятия"
                                      v-mask="'##.##.#### ##:##'" v-model="event.end_at"/>
                        <v-autocomplete v-model="event.event_type_id" :items="$store.state.events.event_types" item-text="name" item-value="id"
                        ...
                        <v-text-field v-model="event.phone" label="Телефон для справки" v-mask="'+7##########'"/>
                        <v-switch label="Командное участие" v-model="event.teams_allowed"/>
                        <v-file-input v-model="header_img" label="Постер"></v-file-input>
                    </v-card-text>
                    <v-card-actions>
                        <v-btn @click="save" color="blue darken-2" outlined>Добавить мероприятие</v-btn>
                    </v-card-actions>
                </v-card>
            </v-col>
            <v-col cols="12" lg="5">
    ...
    export default {
        ...
        methods: {
            ...
            save(){
                ...
                fd.append('header_bg', this.header_img)
